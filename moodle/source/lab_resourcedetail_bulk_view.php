...
require_once(__DIR__ . '/forms/lab_resourceBulkDetailForm.php');
$mform = new labResourceBulkDetailForm();
$mform->render();
//Form processing and displaying is done here
if ($mform->is_cancelled()) {
    //Handle form cancel operation, if cancel button is present on form
    redirect(new moodle_url('../ausleihverwaltung/lab_removeResource.php', array('id' => $cm->id, 'resourceid' => $resource->id)));
} else if ($fromform = $mform->get_data()) {
    //Handle form successful operation, if button is present on form
    redirect(new moodle_url('../ausleihverwaltung/lab_resource_edit_view.php', array('id' => $cm->id, 'resourceid' => $fromform->resourceid)));
} else {
    // this branch is executed if the form is submitted but the data doesn't validate and the form should be redisplayed
    // or on the first display of the form.
    // Set default data (if any)
    // Required for module not to crash as a course id is always needed
    $formdata = array('name' => $resource->name, 'type' => $resource->type, 'category' => $resource->category, 'description' => $resource->description, 'status' => $resource->status, 'quantity' => $resource->quantity, 'available' => $resource->available, 'comment' => $resource->comment, 'id' => $id);
    $mform->set_data($formdata);
    //displays the form
    $mform->display();
}

echo $OUTPUT->single_button(new moodle_url('../ausleihverwaltung/lab_resourcelist_view.php', array('id' => $cm->id)), 'Back');
