...
$PAGE->set_url('/mod/ausleihverwaltung/stdnt_available_resource_view.php', array('id' => $cm->id));
...
echo $OUTPUT->single_button(new moodle_url('../ausleihverwaltung/stdnt_available_resource_view.php', array('id' => $cm->id)), 'New Rental Application');

echo '<br>';

$table = new html_table();
$table->head = array('Type', 'Resource', 'Status', 'Quantity', 'Details');
...
foreach ($SESSION->applicationList as $application){
    ...
    $table->data[] = array($application->applicationtype, $application->resource, $application->status, $application->amount, $htmlLink);
}
